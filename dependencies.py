...
async def initialize_api_components(app_state: AppState):
    ...
    load_dotenv()
    # --- Kiểm tra kết nối tới Redis --- (COMMENTED OUT)
    app_state.process_input_llm = ChatGroq(model=config.GROQ_MODEL_NAME,temperature=0.2)
    if app_state.process_input_llm is None:
        logger.error("🔸Lỗi khi tải Groq model.")
        raise HTTPException(status_code=500, detail="Failed to load Groq model")
    # try:
    ...
    # --- Kiểm tra kết nối tới MongoDB --- (COMMENTED OUT)
    ...
    if app_state.weaviateDB is None:
        ...
        raise HTTPException(status_code=500, detail="Lỗi kết nối tới Weaviate.")
    ...
    app_state.llm = llm
    logger.info(f"🔸Tải LLM (G) thanh cong")
...
# COMMENTED OUT - Authentication not needed for RAG-only API
...
# COMMENTED OUT - Admin function not needed for RAG-only API
