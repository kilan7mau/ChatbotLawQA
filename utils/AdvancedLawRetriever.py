...
class AdvancedLawRetriever(BaseRetriever):
    ...
    def _extract_searchable_keywords_with_llm(self, question: str) -> List[str]:
        ...
        try:
            ...
            keywords = keyword_chain.invoke({"raw_text": question})
    ...
    self, query: str, *, run_manager: CallbackManagerForRetrieverRun
        ...
        try:
            # Use compress_documents for reranking (since .rank() is not available)
            ...
            logger.error(f"Failed to re-rank with cross-encoder: {e}. Returning unranked candidates.")
            return final_candidates_for_rerank[:self.default_k]
