...
from schemas.user import UserOut  # Needed for commented code
# from dependencies import get_current_user  # COMMENTED OUT - No auth needed
from services.chat_service import ask_question_service, stream_chat_generator
# from utils.utils import delete_chat_from_redis  # COMMENTED OUT - No Redis needed
from dependencies import get_app_state
import logging
import uuid
# from redis.asyncio import Redis  # COMMENTED OUT - No Redis needed
from datetime import datetime, timezone
# from db.mongoDB import conversations_collection  # COMMENTED OUT - No MongoDB needed
...
async def chat_message(request_body: QueryRequest, request: Request):
    """Refactored - removed auth dependency"""
...
async def stream_chat_endpoint(
    ...
    """Refactored - removed auth dependency, added chat_history support"""
    app_state = get_app_state(request=request)
    ...
    # COMMENTED OUT - Requires Redis/MongoDB/Auth  
    ...
    # COMMENTED OUT - Requires MongoDB/Auth
    ...
    try:
        ...
        logger.error(f"Error in get_conversations for user {user.email}: {e}",
                     exc_info=True)  # exc_info=True sẽ log cả traceback
        raise HTTPException(status_code=500, detail="An error occurred while fetching conversations.")

    # COMMENTED OUT - Requires MongoDB/Redis/Auth
    ...
    try:
        ...
        with redis_client.pipeline() as pipe:
            ...
            pipe.execute()
        logger.info(
            f"Đã nạp và đồng bộ hội thoại {chat_id} vào Redis với {len(validated_history_for_response)} tin nhắn.")
...
# ===== NEW ENDPOINTS FROM DEMOVER2.PY =====
